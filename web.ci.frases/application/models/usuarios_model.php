...
class usuarios_model extends CI_Model {
	...
	function genEntradasByCurrentDay($maxDate) {
		$sql="SELECT * FROM entrada WHERE DATE(Time) >= '$maxDate' + INTERVAL 1 DAY";
		$q=$this->db->query($sql);		
	...
	function getLlegadasUsuario($idUsuario) {
		$sql="
			SELECT DISTINCT
				...
				(SELECT tipo FROM retardos WHERE DATE(fecha)=DATE(llegadas.hrLlegada) AND idUsuario = llegadas.idUsuario AND idUsuario NOT IN (
						SELECT idUsuario FROM permisosusuarios WHERE fecha=DATE(llegadas.hrLlegada)
						)) AS retardo				
			FROM 
				llegadas 
			WHERE 
		...
		$sql="
			SELECT DISTINCT
				...
				FROM 
					retardos 
				WHERE 
					DATE(fecha)=DATE(llegadas.hrLlegada) AND 
					idUsuario = llegadas.idUsuario AND 
					idUsuario NOT IN (
						SELECT idUsuario FROM permisosusuarios WHERE fecha=DATE(llegadas.hrLlegada)
						)
					) AS retardo
	...
	function getLlegadasByDateRange($initDate, $endDate) {
		$sql="
			SELECT DISTINCT
				(SELECT nombre FROM usuarios WHERE id = llegadas.idUsuario) AS nombre,
				idUsuario,
				SUM(multa) AS multa,
				SUM(ROUND(TIME_TO_SEC(diferenciaMin)/60, 1)) AS minutosTarde,
				(SELECT COUNT(*) FROM retardos WHERE idUsuario=llegadas.idUsuario AND DATE(fecha) >= '".$initDate."' AND DATE(fecha) <= '".$endDate."' AND tipo=1 AND idUsuario NOT IN (SELECT idUsuario FROM permisosusuarios WHERE fecha = DATE(llegadas.hrLlegada))) AS retardos,
				(SELECT COUNT(*) FROM retardos WHERE idUsuario=llegadas.idUsuario AND DATE(fecha) >= '".$initDate."' AND DATE(fecha) <= '".$endDate."' AND tipo=2 AND idUsuario NOT IN (SELECT idUsuario FROM permisosusuarios WHERE fecha = DATE(llegadas.hrLlegada))) AS faltas
			FROM
				llegadas
			WHERE
				DATE(hrLlegada) >= ? AND
				DATE(hrLlegada) <= ?
				GROUP BY idUsuario, nombre, idEtapa
				ORDER BY multa DESC		
		";
		$q=$this->db->query($sql, array($initDate, $endDate));
	...
	function getLastDateEntrada() {
		$sql="SELECT MAX(DATE(Time)) AS fecha FROM entrada limit 1";
		$q=$this->db->query($sql);		
		
		return ($q->num_rows() > 0) ? $q->row() : false;
	}
	
	function getDalaysUser($type, $idUsuario, $initDate, $endDate) {
		$sql="
			SELECT
				(SELECT nombre FROM usuarios WHERE id = retardos.idUsuario) AS nombre,
				(SELECT nombre FROM horarios WHERE id = retardos.idHorario) AS horario,
				fecha AS fecha
			FROM
				retardos
			WHERE
				DATE(fecha) >= '".$initDate."' AND
				DATE(fecha) <= '".$endDate."' AND
