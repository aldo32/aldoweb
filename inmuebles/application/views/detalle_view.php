<!DOCTYPE html>
<html>
<head>

    <title>Detalle</title>

    <?php echo $includes; ?>

    <script>
    $(document).ready(function() {
        $('#coinslider').coinslider({width: 780, height: 370, delay: 5000,});
    });
    </script>


</head>

<body id="listado" class="Mexico panel-usuario" itemscope="" itemtype="">
<header class="header  -home clearfix oculto-print" style="background: rgba(100, 174, 227, 0.9) none repeat scroll 0 0">
    <div class="container">
        <!--LOGO-->
        <div class="logo-header-wrap" style="width: 100px;">
            <a href="#" title="" class="logo-header"></a>
        </div>

        <div class="header-searchbox-wrap">
            <div style="position: relative; width: 100%">
                <div style="float: left; margin-right: 15px;"><input type="text" name="search" placeholder="Busca tu inmueble" style="width: 990px; margin-top: 0px;"></div>
                <div style="float: left; margin-top: 2px;"><button data-url-destino="#" data-action="buscar" class="btn btn-noframe">Buscar</button></div>
            </div>
        </div>
    </div>
</header>

<div class="content" data-aviso="1011368">
    <div class="container">
        <div class="row"></div>
        <div class="row">
            <div class="span24"></div>
        </div>

        <div class="row sticky-holder">
            <div class="span16">
                <div class="card">
                    <div class="card-content no-padding-bottom">
                        <div class="clearfix">
                            <!-- TITULO -->
                            <h1>La Capital</h1>
                        </div>
                    </div>

                    <!-- GALERIA -->
                    <div class="royalSlider" id="coinslider">
                        <img src='<?php echo base_url();?>resources/images/24302921_002.jpg' style="width: 780px; height: 370px;" width="780" height="370">
                        <img src='<?php echo base_url();?>resources/images/home-img-1.jpg' style="width: 780px; height: 370px;" width="780" height="370">
                    </div>

                    <div class="card-content">
                        <!-- BRADCRUMBS -->
                        <div itemprop="breadcrumb">
                            <ul class="breadcrumb no-margin pull-left oculto-print">
                                <li><a href="http://www.inmuebles24.com/">Inmuebles</a><span class="divider"></span>
                                </li>
                                <li><a href="http://www.inmuebles24.com/departamentos.html">Departamento</a><span
                                        class="divider"></span></li>
                                <span>La Capital</span>
                            </ul>
                        </div>
                    </div>
                </div>

                <div class="row">
                    <div class="span8">
                        <!-- Switch de unidad al desarrollo -->
                        <!-- DATOS -->
                        <div class="card aviso-datos"><!-- ANTES: caracteristicas-aviso -->
                            <div class="card-content">
                                <h3>Datos principales</h3>
                                <ul>
                                    <li>
									    <span class="nombre"><i class="licon licon-ficha_precio"></i>Precio  desde</span>
                                        <span class="valor">MN 9,700</span>
                                    </li>
                                    <li>
                                        <span class="nombre"><i class="licon licon-dormitorios"></i>1 - 2 Recámaras</span>
                                    </li>
                                    <li>
                                        <span class="nombre"><i class="licon licon-banos"></i>Hasta 1 Baños</span>
                                    </li>
                                    <li>
                                        <span class="nombre"><i class="licon licon-garages"></i>Hasta 1 Estacionamientos</span>
                                    </li>
                                </ul>
                            </div>
                            <!-- DATOS -->
                            <div class="card-content">
                                <ul>
                                    <li>
                                        <span class="nombre">Tipo de desarrollo</span>
                                        <span class="valor" title="Vertical">Vertical</span>
                                    </li>
                                    <li>
                                        <span class="nombre">Entrega</span>
                                        <span class="valor" title="Entrega Inmediata">Inmediata</span>
                                    </li>
                                    <li>
                                        <span class="nombre">Unidades</span>
                                        <span class="valor" title="190 Unidades">190</span>
                                    </li>
                                    <li>
                                        <span class="nombre">Desde</span>
                                        <span class="valor" title="Desde MN 9,700">MN 9,700</span>
                                        <br>
                                        <button class="btn btn-link no-padding-left"></button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div class="card aviso-desde">
                            <div class="card-content">
                                <h3 class="no-margin">Publicado hace 32 días</h3>
                            </div>
                        </div>
                        <!-- MAPA -->
                        <div class="card location">
                            <div class="card-content no-padding-bottom">
                                <h3>Ubicación</h3>
                                <div class="list list-directions">
                                    <ul>
                                        <li>Washington 1400, Centro, Monterrey, NL., Centro</li>
                                    </ul>
                                </div>
                            </div>
                            <input id="idGeoloc" name="idGeoloc" value="1011368" type="hidden">
                            <input id="lat" name="lat" value="25.672699506691334" type="hidden">
                            <input id="lng" name="lng" value="-100.299360983375550" type="hidden">
                            <input id="southwest" name="southwest" value="" type="hidden">
                            <input id="zoom" name="zoom" value="18" type="hidden">

                            <div id="imgVar" class="clicvermapa" data-text="Haga click para habilitar">
                                <img src="<?php echo base_url() ?>resources/images/map-holder.png"
                                     style="width:780px;height:300px">
                            </div>
                        </div>
                    </div>
                    <div class="span8">
                        <!-- DESCRIPCIÓN -->
                        <div class="card description">
                            <div class="card-content">
                                <h3 class="clearfix">Descripción</h3>
                                <span class="description-body " id="id-descipcion-aviso">
								    Es un proyecto inmobiliario único en la zona, pioneros en impulsar por medio de una propuesta vanguardista, un nuevo estilo de vida en el corazón de Monterrey, capaz de adaptarse a las necesidades de una sociedad dinámica y de una manera autentica.&nbsp; <br><br>Cuenta con un área comercial en la que podrás encontrar: Helados Sultana, Oriental Wok, TODO EMPANADAS y Oxxo. <br><br>&nbsp; <br><br>Encuéntranos en: www.vyvelacapital.com.mx
                                </span>
                            </div>
                        </div>
                        <!-- CARACTERISTICAS -->
                        <div class="card aviso-caracteristicas">
                            <div class="card-content">
                                <h3>Áreas Sociales</h3>
                                <div class="list list-checkmark no-margin">
                                    <ul>
                                        <li>Lobby</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="card aviso-caracteristicas">
                            <div class="card-content">
                                <h3>Exteriores</h3>
                                <div class="list list-checkmark no-margin">
                                    <ul>
                                        <li>Estacionamiento de visitas</li>
                                        <li>Roof Garden</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="card aviso-caracteristicas">
                            <div class="card-content">
                                <h3>Ambientes</h3>
                                <div class="list list-checkmark no-margin">
                                    <ul>
                                        <li>Business Center</li>
                                        <li>Salón de usos múltiples</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="card aviso-caracteristicas">
                            <div class="card-content">
                                <h3>Generales</h3>
                                <div class="list list-checkmark no-margin">
                                    <ul>
                                        <li>Elevador(es) <span class="coment">(2)</span></li>
                                        <li>Alberca</li>
                                        <li>Acabados de Lujo</li>
                                        <li>Seguridad</li>
                                        <li>Escuelas Cercanas</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="card aviso-caracteristicas">
                            <div class="card-content">
                                <h3>Servicios</h3>
                                <div class="list list-checkmark no-margin">
                                    <ul>
                                        <li>Gimnasio</li>
                                        <li>Internet/Wifi</li>
                                        <li>Circuito Cerrado</li>
                                        <li>Seguridad contra incendios</li>
                                        <li>Planta de emergencia</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="span8">
                <!-- PRECIOS -->
                <div class="card zona-precios">
                    <div class="card-content">
                        <p class="h2 precios no-margin">
                            <span class="nombre">Precio  Desde</span>
                            <span class="valor pull-right" title="Desde MN 9,700">MN 9,700</span>
                        </p>
                        <div class="precios">
                        </div>
                    </div>
                </div>
                <!-- FORMULARIO -->
                <div style="z-index: 10; position: sticky; top: 20px;" class="sticky">
                    <div class="card aviso-contacto">
                        <div class="card-content form-contacto-wrap" style="color: #000;">
                            <div class=" ">
                                <div class="form-contacto contacto-propiedad module">

                                    <div class="consultarAviso-formulario">
                                        <form class="consultarAviso" id="id-consultar-aviso-1">
                                            <p class="valError hide"></p>
                                            <input name="idContacto" value="" type="hidden">

                                            <div class="control-group">
                                                <div class="controls">
                                                    <textarea name="mensaje" placeholder="Mensaje"
                                                              class="input-block open-form">Estoy interesado en esta propiedad y quiero recibir más información. Muchas gracias.</textarea>
                                                </div>
                                            </div>
                                            <div class="control-group">
                                                <div class="controls">
                                                    <input name="nombre" placeholder="Nombre y Apellido"
                                                           class="input-block " maxlength="180" type="text">
                                                </div>
                                            </div>
                                            <div class="control-group">
                                                <div class="controls">
                                                    <input name="telefono" placeholder="Teléfono" class="input-block "
                                                           maxlength="50" type="text">
                                                </div>
                                            </div>
                                            <div class="control-group">
                                                <div class="controls">
                                                    <input required="" name="email" placeholder="E-mail"
                                                           class="input-block close-form" id="emailFormConsulta"
                                                           maxlength="110" type="email">
                                                </div>
                                            </div>
                                            <div class="control-group clearfix">
                                                <button type="submit" class="btn btn-primary btn-large btn-block"
                                                        data-action="consultaraviso" data-tipo-anunciante="lancamentos">
                                                    Contactar anunciante
                                                </button>
                                                <small class="coment no-margin" style="color: #000;">
                                                    Al hacer click en "Enviar" aceptas los
                                                    <a href="http://www.inmuebles24.com/terminos.bum" target="_blank">
                                                    términos y condiciones</a> de Inmuebles2
                                                </small>
                                            </div>
                                        </form>
                                        <div class="consultarAviso-enviando">
                                            <div>
                                                <img
                                                    src="<?php echo base_url()  ?>resources/images/ajax-loader-2k.gif"
                                                    height="52" width="52">

                                                <p>Enviando mensaje...</p>
                                                <span class="a cancelarEnvio">Cancelar</span>
                                            </div>
                                        </div>
                                        <div class="consultarAviso-enviado">
                                            <div>
                                                <i class="ticon ticon-ready"></i>

                                                <p>Mensaje enviado</p>
                                                <span class="a cancelarEnvio">Enviar otro mensaje</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<?php echo $footer; ?>

</body>
</html>