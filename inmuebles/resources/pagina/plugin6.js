...
//mudanca
	$('.btn-calculadora-mudanca').on('click',function(){ 
		...
		doGoogleEvent('Guia de mudança', 'Orçar mudança', tipoProp);
...
//Calculadora general
	$('.js-btn-calc').on('click',function(e){
		e.preventDefault();
		// Los datos personales los tiene que traer solos si el usuario está logeado.
		...
		if (bumexFeature.isFeatureHabilitadaEnPaisYEnAmbiente("CALCULADORA_GENERAL",config.idPais,config.ambiente)) {
			doGoogleEvent('Calculadora','Calcular crédito',labelEventGACalculadora);
		}
		//
...
//			s.ev.on('rsExitFullscreen', function() {
				// Vuelvo a cambiar el modo de escalado al original y refresco el tamaño para que se aplique el scale
        	...
        	realestate(e).propiedades.reporte.reportar({
        		...
        		success: function(contenido){
        			...
        			if (contenido.codigo == 200) {
            			...
            			realestate().ui.notification.add({text:traductor.get("ficha_aviso_reportado","Se reportó correctamente")});
		...
		if(checked){
			$(".grid-posts-multilead .grid-posts .post-action-check input:checked").parents('.post').each(function(e) {
				var settings = {
								...
								case 200:
									...
									setTimeout(function(){
										if ($('.ui-pnotify').length < 1){
											realestate().ui.notification.add({text:traductor.get("ficha_mas_por_mail","Recibirás más información de estos avisos por e-mail"), type:"success"});
											$(".multilead-wrap").removeClass('open');
										}
									},50);
						...
						error: function(){
							$this.text(traductor.get("ficha_mas_info","Receber mais detalhes destes imóveis")).prop('disabled',false);
							realestate().ui.notification.add({text:traductor.get("ficha_js_error","Error"), type:"error"});
						}
				};
				setTimeout(function(){//FIXME: esto es para que se marque deshabilite el boyon antes del ajax, pero habría que revisar por que dispara el ajax antes de tiempo.
					realestate(e).propiedades.contacto.verdatosanunciante(settings);
				},1);
			});
		}else{
...
function setMapa(lat,lng,zoomLevel){
	...
	var marker = new google.maps.Marker({position: new google.maps.LatLng(lat, lng), map: mapa });//FIXME: Me llaman en algún momento?
}
