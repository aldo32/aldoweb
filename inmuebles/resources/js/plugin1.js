...
(function($) {
			...
			pnotify.pnotify_position = function(dont_skip_hidden){
				...
				if (!hidden || dont_skip_hidden) {
					...
					if ((s.dir1 == "down" && s.nextpos1 + pnotify.height() > jwindow.height()) ||
						...
						s.nextpos1 = s.firstpos1;
						s.nextpos2 += s.addpos2 + (typeof s.spacing2 == "undefined" ? 15 : s.spacing2);
						s.addpos2 = 0;
					}
	...
	$.pnotify.defaults = {
		...
		stack: {"dir1": "down", "dir2": "left", "push": "bottom", "spacing1": 15, "spacing2": 15}
	};
...
$.extend($.validator, {
	...
	messages: {
		...
		range: $.validator.format("Ingrtese un valor entre {0} y {1}."),
		max: $.validator.format("Ingrese un valor menor o igual a {0}."),
		min: $.validator.format("Ingrese un valor superior a {0}.")
	},
...
}(jQuery));//MARCADORES INTERESADO NUEVOS
// Necesita tooltip de bootstrap 
$.fn.marcaNumerica = function(options){
	var $this = this;
	var settings = $.extend({
        bgColor: "red",
        textColor: "white",
	...
	$this.each(function(){
		var cn = $(this).attr('data-nuevos');
		var ti = $(this).attr('data-title');
		var t = $(this).attr('data-nuevos-top');
		var b = $(this).attr('data-nuevos-bottom');
		var l = $(this).attr('data-nuevos-left');
		var r = $(this).attr('data-nuevos-right');
		var tp = $(this).css('position');
		var id = $(this).data('nuevos-id');
		
		if(tp != "relative" || tp != "absolute" || tp != "fixed" ){
			$(this).css('position','relative');
		}
		
		if (ti == null || ti == undefined || ti == ""){
			tit = "";
		}else{
			tit = 'data-toggle="tooltip" title="'+ti+'"';
		}
		if(cn != null && cn != undefined && cn != ""){
			var sId = id !=null && id != undefined && id != '' ? (" id='"+id+"'"):"";
			...
			if(top == "auto" && bottom == "auto" && left == "auto" && right == "auto"){
				$(this).append('<div class="bolita-nuevos" style="background:'+
						settings.bgColor+';color:'+
						settings.textColor+';font-size:'+
						settings.fontSize+';border-width:'+
						settings.borderWidth+';opacity:0; top:-8px; right: 0;"'+
						tit+'" '+sId+'>'+cn+'</div>');
			}else{
				$(this).append('<div class="bolita-nuevos" style="background:'+
						settings.bgColor+';color:'+
						settings.textColor+';font-size:'+
						settings.fontSize+';border-width:'+
						settings.borderWidth+';opacity:0; top:'+top+'; right:'+right+'; bottom:'+bottom+'; left:'+left+';"'+
						tit+'" '+sId+'>'+cn+'</div>');
			}
			$('.bolita-nuevos').css('display','block');
			var t = setTimeout(function(){
				$('.bolita-nuevos').css('opacity','1');
				$('.bolita-nuevos').parent().css('opacity','1');
			},settings.delay);
		}
	});
	//inicio tooltip
	$('[data-toggle="tooltip"]').tooltip({placement:"bottom"});
};$.fn.preloader = function(action, options){
	if(!action && !options ){
		options = {};
		action = "show";
	}else
	...
	var	position = ($this.selector == 'html') ? "fixed":"absolute";
	if(action == "hide"){
		// Mato al preloader
		$('.nt-preloader').remove();
		// Vuelvo a habilitar campos
		$this.find('.nt-disabled').removeAttr('disabled').removeClass('nt-disabled');
	}else{
		var w = $this.width(); // Ancho de contenido
		var h = $this.height(); // Alto de contenido
		var tp = $this.css('position');
		var th = settings.fontSize + 5; // Alto del contenedor de texto
		...
		// Seteo position
		if(tp == "static" || tp == "inherit" || tp == "initial" ){
			$(this).css('position','relative');
		}


		// Construyo el preloader
		var mu =	'<div class="nt-preloader" style="position:'+position+';width:'+w+'px;height:'+h+'px;z-index:'+zi+1+';top:0;left:0;">'+
						'<div style="position:absolute;width:'+w+'px;height:'+h+'px;border:'+settings.border+'; padding:10px; margin:'+settings.top+'px '+settings.right+'px '+settings.bottom+'px '+settings.left+'px; background:'+settings.background+';opacity:.'+settings.alpha+';z-index:'+zi+2+';top:0;left:0;"></div>'+
							'<div style="position:absolute;z-index:'+zi+3+';color:'+settings.color+'; width:100%; font-size:'+settings.fontSize+'px; height:'+th+'px; text-align:center; top:50%; margin-top:-10px;">'+
							settings.text+
							'</div>'+
					'</div>';
		
		// Defino elemento padre evitando que sea html
		var	ap = ($this.selector == 'html') ? "body" : $this.selector;
		
		// Deshabilito a todos los elementos si es un formulario o parte de uno
		$this.find('input').not('[disabled]').attr('disabled', true).addClass('nt-disabled');
		$this.find('textarea').not('[disabled]').attr('disabled', true).addClass('nt-disabled');
		$this.find('select').not('[disabled]').attr('disabled', true).addClass('nt-disabled');
		$this.find('button').not('[disabled]').attr('disabled', true).addClass('nt-disabled');

		// Quito preloader si este elemento o el padre ya tiene un preloader
		$this.find('.nt-preloader').remove();
		$this.parents('.nt-preloader').remove();
		// Agrego preloader al elemento correspondiente
		$(ap).append(mu);
	}	
...
f.fn.cleanVal=function(){return this.data("mask").getCleanVal()};f("*[data-mask]").each(z);f(document).on("DOMNodeInserted.mask","*[data-mask]",z)});/*! Magnific Popup - v0.8.9 - 2013-06-04
...
//HACER COSAS
$(function(){
	$(document).on('click','a.btn-close',function(e){
		realestate(e).ui.popup.close();
	});
});